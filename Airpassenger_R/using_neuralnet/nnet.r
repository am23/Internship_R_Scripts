#This code helps to train and test Airpassengers data using neural networks. It uses the 'carat' package which has a 'train' function. It predicts values from 1960 to 1965.
# It generates  the image image_nnet.png

#Reading files
a<-read.table('original+dummy.txt')
b<-read.table('original.txt')
...
x <- ts(a, start=c(1949,1),frequency=12)   #data for predicting
y <- ts(b, start=c(1949,1),frequency=12)   #used for training
...
legend("topleft",cex=1, pch=18,col=c("red","blue"),legend=c("Original","Predicted"))
